  ...
  return (
    <div className="App">
      <header className="App-header">
        <h1>SEC 재무 데이터 뷰어</h1>
        {/* isSearching prop은 FinancialDataTable의 isLoading 상태를 전달할 수 있으나, 여기서는 생략 */}
        <SearchForm onSearch={handleSearch} onReset={handleReset} isSearching={false} />
      </header>
      <main className="content">
        {/* searchParams가 있을 때만 데이터 테이블을 렌더링 */}
        {searchParams ? (
          <FinancialDataTable 
          ...
          <p style={{ textAlign: 'center' }}>📈 회사와 기간을 선택하고 검색 버튼을 눌러주세요.</p>
        )}
      </main>
    </div>
  );
